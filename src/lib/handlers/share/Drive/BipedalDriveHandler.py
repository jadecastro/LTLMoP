...
class BipedalDriveHandler(handlerTemplates.DriveHandler):
    ...
    def setVelocity(self, v, w, theta=0):
        ...
        if v == 0.:
            vxout = 0.              # Don't move
            wout = 0.
        vxout = min(max(v,0),self.maxspeed)
        wout = min(max(w,-self.maxspeed),self.maxspeed)

        # if v < self.minvel and w == 0.:
        #     vxout = 0.              # Don't move
        #     wout = 0.
